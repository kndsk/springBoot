...
public class ZipUtilsForScale {
  
  public static final String EXT = ".rpk";
   ...
   * @throws Exception
   */
  public File compress(File srcFile) throws Exception {
    String name = srcFile.getName();
    String basePath = srcFile.getPath().substring(0, srcFile.getPath().lastIndexOf(File.separator));
    String destPath = basePath + File.separator + name + EXT;
    File file = compress(srcFile, destPath);
    return file;
  }
   ...
   * @throws Exception
   */
  public File compress(File srcFile, String destPath) throws Exception {
    File file = new File(destPath);
    compress(srcFile, file);
    return file;
  }
   ...
   * @throws Exception
   */
  public File compress(String srcPath) throws Exception {
    File srcFile = new File(srcPath);
    
    File file = compress(srcFile);
    return file;
  }
  ...
  /**
   * 压缩文件
   * @param srcfile FileList 需要压缩的文件列表
   * @param zipfile File    压缩后的文件
   * @param response 
   */
    public static void ZipToResponse(List<File> srcfile, java.io.File zipfile, String filePath) {
