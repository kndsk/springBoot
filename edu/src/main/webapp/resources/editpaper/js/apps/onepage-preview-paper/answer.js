...
define(function(require, exports, module) {
	...
	var answer = {
				...
				$t.find('.checkbox-normal').each(function(){
					 ...
					 if($(this).hasClass('checkbox-selected')){
						 ...
						 if ($(this).parent().find('.ht-mk') != null && $(this).parent().find('.ht-mk').length != 0){
							 ...
							 //对其他选项中包含空的进行检验
							 $(this).parents('.preview-wrapper').find('.checkbox-selected').each(function(){
								 if ($(this).parent().find('.ht-mk') != null && $(this).parent().find('.ht-mk') != ''){
									 var flag = true;
									 $(this).parent().find('.ht-mk').each(function(){
										 if($(this).val() == ''){
											 flag = false;
		...
		//添加已作答试题题号至window.areadyAnswerQuestion（不重复添加）
		addHadAnswerQNum:function(qnum){
			var flag = false;
			for(var i = 0; i < window.areadyAnswerQuestion.length; i++){
				if(window.areadyAnswerQuestion[i] == qnum){
					flag = true;
					break;
				}
			}
			if(flag == false){
				window.areadyAnswerQuestion.push(qnum);
			}
		},
		//从window.areadyAnswerQuestion中去除未作答试题题号
		removeNotAnswerQNum:function(qnum){
			var flag = false;
			for(var i = 0; i < window.areadyAnswerQuestion.length; i++){
				if(window.areadyAnswerQuestion[i] == qnum){
					window.areadyAnswerQuestion.splice(i, 1);
				}
			}
		},
		//获取作答数据
			...
			if (type === '5') {
				$t.find('.checkbox-normal').each(function() {
					if ($(this).hasClass('checkbox-selected')) {
						val = $(this).attr('value');
						choiceVal.push(val);
						if ($(this).parent().find('.ht-mk') != null && $(this).find('.ht-mk').parent().length != 0) {
							$(this).parent().find('.ht-mk').each(
								function() {
									if ($(this).val() != '') {
										val = $(this).val();
		...
		updateProgressSliderBar:function(){
			var num = window.areadyAnswerQuestion.length;
			var progressNum = parseInt((num/window.total)*100);
		...
		//缓存数据
		cacheData:function(){
			//将window.areadyAnswerResult,window.areadyAnswerQuestion存入到本地缓存
			if(window.localStorage){
				var resultKey = window.paperId + '_result';
				var questionKey = window.paperId + '_question';
				localStorage[resultKey] = JSON.stringify(window.areadyAnswerResult);
				localStorage[questionKey] = JSON.stringify(window.areadyAnswerQuestion);
			}
		},
		simulateInit:function(){
	        ...
	        if(window.localStorage && localStorage[resultKey] != undefined){
	        	//已作答试题题号集合  
	        	window.areadyAnswerQuestion = eval('('+localStorage[questionKey]+')');
	        	//已作答试题答案集合
		...
		simulateAnswer:function(){
			//此处是采用克隆 而不是赋值指针
			var tempAnswerQuestionNumArr = window.areadyAnswerQuestion.slice();
				...
				if($this.attr('type') != '11'){
					...
					for(var i = 0; i < window.areadyAnswerResult.length; i++){
						if(code === window.areadyAnswerResult[i].questionNum){
							...
							var index = $.inArray(code, tempAnswerQuestionNumArr);
							if(index != -1){
								tempAnswerQuestionNumArr.splice(index, 1);
							}
						}
					}
