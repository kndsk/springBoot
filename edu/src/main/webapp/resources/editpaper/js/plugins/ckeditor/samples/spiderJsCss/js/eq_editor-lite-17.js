...
var Panel = {
    ...
    hover: function(img, e) {
        if (this.hoverdiv) {
            ...
            this.hoverdiv.innerHTML = '<img src="' + EQUATION_ENGINE + '/latex?\\200dpi l= ' + img.latex + '"/>';
            if (document.all) {
...
function EqTextArea(preview, input, comment, download, intro) {
    ...
    this.setText = function(val) {
        var latex = decodeURI(val.replace(/\&space;/g, ' ').replace(/\&plus;/g, '+').replace(/\&hash;/g, '#').replace(/\@plus;/g, '+').replace(/\@hash;/g, '#'));
        EqEditor.reset();
        var i, subtex, go;
        do {
            go = 0;
    ...
    this.exportEquation = function(type) {
        ...
        case 'safe':
            ...
            break;
        case 'encoded':
            return encodeURI(this.getEquationStr()).replace(/\+/g, '&plus;');
            break;
        case 'wp':
            {
        ...
        case 'urlencoded':
            {
                ...
                return ( EQUATION_ENGINE + '/latex?' + this.exportEquation('encoded')) ;
        ...
        if (bracket == 0) {
            ...
            if (this.changed) {
                ...
                case 'svg':
                	...
                	img.src = EQUATION_ENGINE + '/latex?' + val;
...
var Example = {
    ...
    add_fav: function() {
        ...
        if (text != '') {
            this.load_json('favorite_json.php', 'sid=' + EqEditor.SID + '&add&eqn=' + encodeURI(text.replace(/\+/g, "@plus;").replace(/#/g, '@hash;')));
    ...
    add_history: function(text) {
        if (text != '') {
            this.load_json('history_json.php', 'sid=' + EqEditor.SID + '&add&eqn=' + encodeURI(text.replace(/\+/g, "@plus;").replace(/#/g, '@hash;')));
        }
    },
