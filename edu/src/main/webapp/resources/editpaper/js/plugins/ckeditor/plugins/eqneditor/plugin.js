...
CKEDITOR.plugins.add( 'eqneditor', {
	...
	init : function(editor)
	{
	  ...
	  // var host ='localhost:8080/paper/ckeditor/samples/spiderJsCss';
	  // var http=('https:' == document.location.protocol ? 'https://' : 'http://');
	  var hostHttp =basePath +'/resources/editpaper/js/plugins/ckeditor/samples/spiderJsCss';

		// First make sure we have loaded the necessary scripts
  	CKEDITOR.scriptLoader.load( [
		//http+host+'/js/eq_config.js',
		//http+host+'/js/eq_editor-lite-17.js',
		hostHttp+'/js/eq_config.js',
		hostHttp+'/js/eq_editor-lite-17.js',
		]);
		...
		fileref.setAttribute("href", hostHttp+'/css/equation-embed.css');
		...
		editor.addCommand(pluginCmd, new CKEDITOR.dialogCommand(pluginCmd,
			{
				allowedContent: 'img[src,data-latex]',
				requiredContent: 'img[src,data-latex]'
			})
		);
		...
		if (editor.contextMenu)
		{
			...
			editor.contextMenu.addListener( function(element) {
				...
				if (element.getAscendant('img', true))
				{
					...
					var sName = element.getAttribute('src').match( /latex\?(.*)/ );
					if(sName!=null)
					{
		...
		editor.on( 'doubleclick', function(evt)
		{
			...
			if (element && element.is('img'))
			{
				//var sName = element.getAttribute('src').match( /(gif|svg)\.latex\?(.*)/ );
				//var sName = element.getAttribute('src').match( /latex\?(.*)/ );
				var sName =element.getAttribute("data-latex");
				if(sName!=null)
				{
